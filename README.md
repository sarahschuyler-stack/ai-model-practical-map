...
3. **Prompt forge.** Optional, and powered by Claude Fable 5.1 — the deepest reasoning model on the map. With an Anthropic API key, the built-in draft becomes raw material in turn: Fable is briefed as a principal prompt engineer, given the job in the requester's own words, the wizard answers and a profile of the target model, and asked for the brief a doctorate-level specialist in that field would write. Both versions stay side by side and the draft is never discarded, so a failed or unavailable call costs nothing. See [The prompt forge](#the-prompt-forge).
4. **Recheck.** Ask an AI to search the web for price, capability and availability changes since the snapshot date, review the findings one by one, and apply the ones you trust. Applied changes are stored in your browser and replayed on top of the published snapshot every load. Reset returns to the published snapshot.
...
| `test/forge.test.mjs` | The Fable pass: the draft shows first, the call goes to Fable without the search tool and with the authoring contract, both versions stay editable, a failure or a missing key leaves the draft intact, a target switch drops the stale brief, fenced replies are unwrapped |
...
The built-in engine is deterministic and free, and it always runs first. Its output is the **draft**. The forge panel above the prompt box hands that draft to **Claude Fable 5.1** and asks for something better.

- **Why Fable.** Writing a prompt is the ambiguous, high-stakes, one-shot reasoning job the map itself scores Fable highest on. The target model is whatever the user picked; the author is always Fable (`FORGE_MODEL`), because the author's job is harder than the executor's.
- **What Fable is told.** A system contract (`FORGE_SYSTEM`) holds it to seven standards: fidelity to the requester's words, real decomposition, testable acceptance criteria, named failure modes (domain traps and model traps alike), explicit epistemics, fit to the target model's profile, and economy. The user message (`forgeBrief`) carries the target model's name, practical scores and guidance, the job verbatim, what `analyzeJob` detected, the wizard answers, and the draft — described as the floor, not the ceiling.
- **What comes back.** The finished prompt and nothing else. A reply that arrives wrapped in a code fence despite the contract is unwrapped by `stripFence()`.
- **Nothing is lost.** The toggle switches between the Fable version and the built-in draft, and edits to each survive the switch. A failure, a missing key, a refusal or a timeout leaves the draft exactly where it was and says so in one line. A brief is written for one target and one set of answers, so changing either drops it rather than showing a brief tuned for a model you are no longer targeting.
- **The call.** Same path as Recheck Option A (`callClaude`, key to `api.anthropic.com` only) with `search:false` — authoring needs no web search — plus the system contract and a 12,000-token ceiling.
...
- **API key.** Sent only to `api.anthropic.com`, by Recheck Option A and by the prompt forge; one key serves both fields. If you tick "remember", it is kept in `sessionStorage` for the current tab and cleared when the tab closes. It is never written to `localStorage`, and any key an earlier version left there is removed at boot.
